...
def get_or_create_public_calendar(service, calendar_name):
...
def add_outage_event(service, calendar_id, start_dt, end_dt, updated, updated_message):
  description = f'''За графіком львівобленерго (автоматично)
Світла не буде {str(end_dt-start_dt)}

{updated_message} (@{updated})
source: https://poweron.loe.lviv.ua/
'''
  event = {
...
def remove_outage_event(service, calendar_id, event_id):
  try:
    service.events().delete(calendarId=calendar_id, eventId=event_id).execute()
  except Exception as e:
    print(f"Error during removing event {e}")

def update_outage_event(service, calendar_id, event_id, updated, updated_message):
...
'''
  ...
  try:
    ...
    print(f"Error during updating evnet {event_id}: {e}")
  


def remove_calendar(service, calendar_id):
  try:
    service.calendars().delete(calendarId=calendar_id).execute()
  except Exception as e:
    print(f"Error during removing calendar {e}")
